...
const HandleMsg = async (message, browser, client = new Client()) => {
    ...
    const resMsg = {
        ...
        success: {
            ...
            greeting: `Hai guys 👋 perkenalkan saya HameBot 🤖` +
    ...
    try {
        ...
        const stickerMetadata = { pack: 'Created with', author: 'HameBot', keepScale: true }
        const stickerMetadataCircle = { pack: 'Created with', author: 'HameBot', circle: true }
        const stickerMetadataCrop = { pack: 'Created with', author: 'HameBot', cropPosition: 'center' }
        // Bot Prefix Aliases
        const regex = /^[\\/!$^%&+.,-](?=\w+)/
        ...
        const doSimi = async (inp) => {
            ...
            if (respon) {
                ...
                reply('▸ ' + respon.replace(/\b(simi|simsim|simsimi)\b/ig, 'hame'))
            }
        }
        ...
        switch (true) {
            ...
            case /\b(bot|hame|hamebot)\b/ig.test(chats): {
                if (!isCmd) {
